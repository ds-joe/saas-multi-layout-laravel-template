...
const UserDropdown: RC = () => {
  ...
  const avatarFallback = ((user: UserModel) => `${user.name[0]}`.toLocaleUpperCase())(auth.user);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="min-w-40">
        <DropdownMenuItem className="text-base"> <HiOutlineUser /> {page_words?.profile}</DropdownMenuItem>
        <DropdownMenuItem className="text-base" onClick={logout}><HiArrowLeftOnRectangle /> {page_words?.logout}</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
}
