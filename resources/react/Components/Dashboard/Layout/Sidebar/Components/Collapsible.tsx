...
// Types
import { CollapsibleProps, CollapsibleItemProps } from "@/types/Components/Dashboard/Layout/Sidebar";
      ...
      <div ref={ref}>
        ...
        >
          <div className="flex justify-between items-center text-sm ">
            ...
            <HiChevronUp className={`transition-transform ${isOpen ? "rotate-180" : ""}`} />
          </div>
        </button>
        <ul
          className={cn(
...
const CollapsibleItem = forwardRef<HTMLLIElement, CollapsibleItemProps>(({ children, active, href, Icon, childItem = true, ...props }, ref) => {
  return (
    <li
      {...props}
      className={cn(
    ...
    >
      <Link
        ...
        href={href ?? "#"}>
        {Icon ? <Icon className={`${childItem ? "text-[1rem]" : "text-[1.1rem]"}`} /> : ""}
        {children}
      </Link>
    </li>
  )
